...
```
🍀 Found 670 error diagnostics are used for compiler tests.
- 1xxx: 200
- 2xxx: 321
- 4xxx: 10
- 5xxx: 26
...
🍀 Found 683 error diagnostics are used for conformance tests.
- 1xxx: 246
- 2xxx: 319
- 4xxx: 12
- 5xxx: 11
...
🍀 Found 928 error diagnostics are used for compiler|conformance tests.
```
...
TSCのコードベースでは、`_namespaces/ts.ts`というやつが長大なBarrel exportをしてて、それを各`parser.ts`やらが利用する形になってるので、依存グラフまで追わないと相互関係がわからない。
...
くらいしないとダメそう。ただそれでも、`checker.ts`が参照してる2xxxシリーズのうち、どれが不要かは判断できない。
